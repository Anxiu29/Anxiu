...
`AppShell` 使用 UI 内部状态切换设备、改键、灯光、高级键、性能、宏和按键测试等工作区的命名插槽。设备会话保持在应用层和 Store 中，不随工作区卸载而销毁。当前导航继续使用本地状态；只有需要可分享链接、刷新恢复页面或浏览器前进后退时，再引入路由，不以页面数量作为重构条件。

`App.vue` 按需加载六个编辑与测试工作区，设备首页仍同步显示；各工作区沿用原有 props 和事件接口。全局 CSS 暂时保持既有顺序，避免懒加载改变主题和响应式规则的覆盖关系。`stores/lightingActions.ts` 独立管理灯光操作，通过共享状态与反馈接口接入主 Store；上下文切换时统一失效其读取和写入结果。
