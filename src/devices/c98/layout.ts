...
const geometryBySourceCode = new Map<number, ControlGeometry>(visualKeys.map((key) => [key.sourceCode, {
  x: key.x,
  y: key.y,
  width: key.width ?? 1,
  height: key.height ?? 1,
...
export const C98_DEMO_KEYS: MatrixKeyInput[] = visualKeys.map((key, index) => ({
  id: `demo-${index}`,
  sourceCode: key.sourceCode,
  label: '',
  address: { kind: 'matrix', row: Math.floor(key.y), column: Math.floor(key.x) },
}))
