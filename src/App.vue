...
const { status, profile, layer, mode, activeConfiguration, selectedPositionId, error, message, dirty, assignments, selectedAssignment, keyOptions, keyLabels, lighting, advancedKey, advancedKeyLoading, advancedKeyTypes, macroSlots, macroLoading, demo, driverId } = storeToRefs(store)
...
<template>
  <main>
    ...
    <AppShell v-else :key="shellRevision" :profile="profile" :active-configuration="activeConfiguration" :navigation-disabled="busy()" :error="error" :message="message" :sidebar-image-url="devicePresentation.sidebarImageUrl" @select-configuration="store.selectConfiguration" @restore-factory="store.restoreFactory">
      ...
      <template #macro><MacroWorkspace :profile="profile" :status="status" :macro-slots="macroSlots" :loading="macroLoading" :assignments="assignments" :key-options="keyOptions" :key-labels="keyLabels" :key-geometry="devicePresentation.keyGeometry" @update="store.updateMacro" /></template>
