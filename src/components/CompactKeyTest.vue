...
function removeKeyAfter(
  target: typeof pressedKeys,
...
<template>
  <aside class="compact-key-test">
    <h3>按键测试</h3>
    <section
      class="compact-key-event pressed"
      :class="{ active: pressedCodes.size > 0 }"
    >
      <span>按下的按键</span>
      <div v-if="pressedKeys.size" class="compact-key-values">
        <strong
          v-for="(label, code) in Object.fromEntries(pressedKeys)"
          :key="code"
          >{{ label }}</strong
        >
      </div>
      <div v-else class="compact-key-test-empty">
        <svg viewBox="0 0 48 48" aria-hidden="true">
          <path
            d="M17 21v-8a3 3 0 0 1 6 0v7-11a3 3 0 0 1 6 0v11-8a3 3 0 0 1 6 0v12l2-2a3 3 0 0 1 4 4l-9 12a9 9 0 0 1-7 3h-3a10 10 0 0 1-8-4L8 29a3 3 0 0 1 5-4l4 5v-9Z"
          />
          <path d="m8 12-4-2m8-5-2-4m9 3V0" />
        </svg>
    ...
    <section class="compact-key-event released">
      ...
      <div v-if="releasedKeys.size" class="compact-key-values">
        <strong
          v-for="(label, code) in Object.fromEntries(releasedKeys)"
          :key="code"
          >{{ label }}</strong
        >
      </div>
