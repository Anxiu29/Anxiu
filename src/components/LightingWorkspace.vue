  ...
  <section class="lighting-workspace">
    ...
    <div v-if="settings" class="lighting-grid">
      <section class="panel lighting-preview" :class="{ off: !settings.open }">
        <div class="lighting-glow" :style="{ '--light-color': settings.colors[0] ?? '#FFFFFF', opacity: settings.open && settings.luminance > 0 ? Math.max(.18, Math.min(1, settings.luminance / 100)) : 0 }"></div>
        <span>MAIN LIGHT</span><strong>{{ settings.open ? '主灯已开启' : '主灯已关闭' }}</strong>
        <small>{{ settings.type === 'static' ? '静态' : settings.type === 'dynamic' ? `动态灯效 ${settings.mode}` : '自定义' }} · 亮度原始值 {{ settings.luminance }}</small>
      </section>
      ...
      <section class="panel lighting-controls">
        ...
        <label><span>亮度原始值 <b>{{ settings.luminance }}</b></span><input type="number" min="0" max="255" :value="settings.luminance" :disabled="busy || !settings.open" @change="update({ luminance: Number(($event.target as HTMLInputElement).value) })" /></label>
