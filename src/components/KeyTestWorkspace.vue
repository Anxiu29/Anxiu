...
interface TestRecord {
  ...
  action: "down" | "up";
  time: string;
  /** 与上一条按下或抬起事件的间隔；界面只在按下记录中展示，抬起使用自身持续时间。 */
  interval?: number;
  /** 当前按键从自身按下到抬起的总时长。 */
  duration?: number;
}
...
const { container: keyboardContainer, unit: keyboardUnit } =
  useFittedKeyboardUnit(
    () => props.profile.positions,
    () => props.keyGeometry,
    { maxUnit: 64, minUnit: 28 },
  );
useHorizontalKeyboardScroll(keyboardContainer);
const totalPresses = computed(() =>
  Object.values(downCounts.value).reduce((sum, count) => sum + count, 0),
);
const totalReleases = computed(() =>
  Object.values(upCounts.value).reduce((sum, count) => sum + count, 0),
);
const testedKeys = computed(
  () =>
    new Set([...Object.keys(downCounts.value), ...Object.keys(upCounts.value)])
      .size,
);
const badges = computed(() =>
  Object.fromEntries(
    [
      ...new Set([
        ...Object.keys(downCounts.value),
        ...Object.keys(upCounts.value),
      ]),
    ].map((positionId) => [
      positionId,
      `${downCounts.value[positionId] ?? 0}/${upCounts.value[positionId] ?? 0}`,
    ]),
  ),
);

function matchingPositions(usage: number) {
  const mapped = props.assignments
    .filter((item) => item.keyCode === usage)
    .map((item) => item.positionId);
  // 没有匹配当前层输出时，用物理 sourceCode 兜底，便于测试尚未出现在键值表中的键。
  return mapped.length
    ? mapped
    : props.profile.positions
        .filter((item) => item.sourceCode === usage)
        .map((item) => item.id);
}
function refreshPressedPositions() {
  pressedPositionIds.value = [
    ...new Set([...pressedByCode.values()].flatMap((item) => item.positions)),
  ];
}
...
function eventInterval(now: number) {
  const interval =
    previousEventAt === undefined ? undefined : now - previousEventAt;
  previousEventAt = now;
  return interval;
}
function handleKeyDown(event: KeyboardEvent) {
  ...
  if (positions.length) {
    ...
    positions.forEach((positionId) => {
      next[positionId] = (next[positionId] ?? 0) + 1;
    });
...
function handleKeyUp(event: KeyboardEvent) {
  ...
  if (pressed.positions.length) {
    ...
    pressed.positions.forEach((positionId) => {
      next[positionId] = (next[positionId] ?? 0) + 1;
    });
    upCounts.value = next;
  }
  ...
  const record: TestRecord = {
    ...
    label: pressed.label,
    code: event.code,
    matched: pressed.positions.length,
    action: "up",
    time: eventTime(now),
    interval: eventInterval(now),
    duration: now - pressed.startedAt,
  };
  history.value = [record, ...history.value].slice(0, 24);
}
function releaseAll() {
...
<template>
  <section class="key-test-workspace">
    <header class="panel key-test-heading">
      <div>
        <span class="eyebrow">KEY TEST</span>
        <h2>按键测试</h2>
        <p>记录浏览器收到的按下、抬起时间以及两种事件的独立次数。</p>
      </div>
      <div class="key-test-stats">
        <span
          ><strong>{{ testedKeys }}</strong> 已测试按键</span
        ><span
          ><strong>{{ totalTriggers }}</strong> 总触发次数</span
        ><span
          ><strong>{{ totalPresses }}</strong> 按下次数</span
        ><span
          ><strong>{{ totalReleases }}</strong> 抬起次数</span
        >
      </div>
    </header>
    ...
    <section class="panel key-test-history">
      <header>
        <div>
          <strong>事件记录</strong
          ><small
            >键帽角标为“按下次数/抬起次数”；Fn 本身可能不会单独上报。</small
          >
        </div>
        <div class="key-test-history-actions">
          <span v-if="pressedPositionIds.length"
            >正在按下 {{ pressedPositionIds.length }} 个键</span
          ><button
            class="ghost"
            type="button"
            :disabled="!history.length && !totalTriggers"
            @click="clearResults"
          >
            清空记录
          </button>
      ...
      <div v-if="history.length" class="key-test-records">
        <article
          v-for="record in history"
          :key="record.id"
          :class="[record.action, { unmatched: record.matched === 0 }]"
        >
          <div>
            <strong>{{ record.label }}</strong
            ><b>{{ record.action === "down" ? "按下" : "抬起" }}</b>
          </div>
          ...
          ><small
            ...
            {{
              record.matched
                ? `匹配 ${record.matched} 个键位`
                : "未匹配当前层键位"
            }}<template
              v-if="record.action === 'down' && record.interval !== undefined"
            >
              · 距上一事件 {{ record.interval }} ms</template
            ><template v-if="record.duration !== undefined">
                · 持续 {{ record.duration }} ms</template
            ></small
          >
        </article>
      </div>
