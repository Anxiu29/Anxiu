...
describe('connected workspace navigation', () => {
  ...
  it('shows the macro workspace only when the device declares macro capability', async () => {
    const macroProfile: KeyboardProfile = { ...profile, capabilities: { ...profile.capabilities, macro: true, macroSlots: 16, macroMaxActions: 42 } }
    const wrapper = mount(AppShell, { props: { profile: macroProfile }, slots: { device: '<div />', macro: '<div data-view="macro">宏编辑器</div>' } })
    const macroButton = wrapper.findAll('.sidebar-nav button').find((button) => button.text().includes('宏设置'))

    expect(macroButton).toBeDefined()
    await macroButton!.trigger('click')
    expect(wrapper.find('[data-view="macro"]').text()).toBe('宏编辑器')
  })
  ...
  it('opens settings as a workspace page and confirms factory reset inline', async () => {
    const wrapper = mount(AppShell, { props: { profile }, slots: { device: '<div />' } })

    await wrapper.find('.sidebar-settings').trigger('click')
    expect(wrapper.find('.settings-workspace').exists()).toBe(true)
    expect(wrapper.find('.settings-navigation').text()).toContain('固件升级')
    expect(wrapper.find('.settings-backdrop').exists()).toBe(false)

    const deviceButton = wrapper.findAll('.settings-navigation button').find((button) => button.text().includes('设备'))!
    await deviceButton.trigger('click')
    await wrapper.find('.factory-reset-button').trigger('click')
    expect(wrapper.emitted('restore-factory')).toBeUndefined()
    expect(wrapper.find('.settings-danger-actions').text()).toContain('再次点击确认')
    await wrapper.find('.factory-reset-button').trigger('click')
  ...
  it('renders device-not-written feedback as a warning', () => {
    const wrapper = mount(AppShell, { props: { profile, message: '宏已保存为未绑定草稿', messageWarning: true }, slots: { device: '<div />' } })

    expect(wrapper.find('.feedback-toast').classes()).toContain('warning')
    expect(wrapper.find('.feedback-toast').classes()).not.toContain('error')
  })

  it('automatically closes feedback after five seconds', async () => {
    vi.useFakeTimers()
    const wrapper = mount(AppShell, { props: { profile, message: '键盘连接成功' }, slots: { device: '<div />' } })
