...
describe('DeviceSession', () => {
  it('keeps the written DKS travels when layout readback only reports calibration travel', async () => {
    const expected = { type: 'dks' as const, sourceCode: 4, keyCodes: [4, 0, 0, 0] as [number, number, number, number], triggers: [1, 0, 0, 0] as [number, number, number, number], travels: [1, 2.4] as [number, number] }
    const session = new DeviceSession({
      profile: { getProfile: async () => profile() },
      advancedKey: {
        getAdvancedKey: async () => ({ ...expected, travels: [1, 3.44] }),
        getAdvancedKeyTypes: async () => ({}),
        setAdvancedKey: async () => undefined,
        deleteAdvancedKey: async () => undefined,
      },
      close: () => undefined,
    }, HID_KEY_CATALOG)

    await expect(session.updateAdvancedKey(expected)).resolves.toEqual(expected)
    await expect(session.getAdvancedKey(4)).resolves.toEqual(expected)
  })
