...
const forbiddenImports = (directory: string, forbidden: RegExp) => filesBelow(join(sourceRoot, directory)).flatMap((path) => {
  const violations = moduleDependencies(path, readFileSync(path, 'utf8'), sourceRoot).filter((specifier) => forbidden.test(specifier))
  return violations.map((specifier) => `${relative(sourceRoot, path)}: ${specifier}`)
})
